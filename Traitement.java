/*@lineinfo:filename=Traitement*//*@lineinfo:user-code*//*@lineinfo:1^1*/
...

{
  ...
  try {
   ...
   __sJT_st = __sJT_ec.prepareOracleStatement(__sJT_cc,"0Traitement",theSqlTS);
   if (__sJT_ec.isNew())
   {
...

{
  ...
  sqlj.runtime.ExecutionContext.OracleContext __sJT_ec = ((__sJT_cc.getExecutionContext()==null) ? sqlj.runtime.ExecutionContext.raiseNullExecCtx() : __sJT_cc.getExecutionContext().getOracleContext());
   ...
   __sJT_st = __sJT_ec.prepareOracleBatchableStatement(__sJT_cc,"1Traitement",theSqlTS);
   // set IN parameters
   __sJT_st.setInt(1,x);
  // execute statement
...

{
  ...
  try {
   String theSqlTS = "select count(*)   from sejour";
   __sJT_st = __sJT_ec.prepareOracleStatement(__sJT_cc,"2Traitement",theSqlTS);
   if (__sJT_ec.isNew())
   {
     __sJT_st.setFetchSize(2);
   }
...

{
  ...
  try {
   ...
   __sJT_st = __sJT_ec.prepareOracleStatement(__sJT_cc,"3Traitement",theSqlTS);
   // set IN parameters
   __sJT_st.setInt(1,x);
   // execute query
   iter = new cons(new sqlj.runtime.ref.OraRTResultSet(__sJT_ec.oracleExecuteQuery(),__sJT_st,"3Traitement",null));
...
//  ************************************************************
//  #sql val = { values (traitement1(:nom,:age))					
//  		 };
//  ************************************************************

{
  ...
  sqlj.runtime.ExecutionContext.OracleContext __sJT_ec = ((__sJT_cc.getExecutionContext()==null) ? sqlj.runtime.ExecutionContext.raiseNullExecCtx() : __sJT_cc.getExecutionContext().getOracleContext());
  try {
   String theSqlTS = "BEGIN :1 := traitement1( :2 , :3 ) \t\t\t\t\t\n\t\t\n; END;";
   __sJT_st = __sJT_ec.prepareOracleCall(__sJT_cc,"4Traitement",theSqlTS);
   if (__sJT_ec.isNew())
   {
      __sJT_st.registerOutParameter(1,oracle.jdbc.OracleTypes.INTEGER);
   }
   // set IN parameters
   __sJT_st.setString(2,nom);
   __sJT_st.setInt(3,age);
  // execute statement
   __sJT_ec.oracleExecuteUpdate();
   // retrieve OUT parameters
   val = __sJT_st.getInt(1); if (__sJT_st.wasNull()) throw new sqlj.runtime.SQLNullException();
